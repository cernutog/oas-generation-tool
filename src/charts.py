...
class SemanticPieChart(ctk.CTkFrame):
    ...
    def draw(self, event=None):
        ...
        others = {
            ...
            if v["severity"] not in ("error", "warning")
        }

        # Sort internal groups by count desc
        errors = dict(
            sorted(errors.items(), key=lambda item: item[1]["count"], reverse=True)
        )
        warnings = dict(
            sorted(warnings.items(), key=lambda item: item[1]["count"], reverse=True)
        )
        ...
        if len(slices_meta) == 1 and slices_meta[0]["is_full"]:
            ...
            self.canvas.create_rectangle(
                cx - rx, cy, cx + rx, cy + depth, fill=s["dark_color"], outline=""
            )
        else:
    ...
    def show_tooltip(self, x, y, slice_data):
        ...
        # Shadow/Bg
        id_txt = self.canvas.create_text(
            tx + 5,
            ty + 5,
            text=text,
            anchor="nw",
