...
class PreferencesDialog(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(self.frame_template, text="Template Directory:").grid(row=0, column=0, sticky="w", padx=(0, 10))
        self.entry_template = ctk.CTkEntry(self.frame_template, placeholder_text="Leave empty for default")
        self.entry_template.grid(row=0, column=1, sticky="ew", padx=(0, 10))
        ctk.CTkButton(self.frame_template, text="Browse", width=70, command=self._browse_template).grid(row=0, column=2)
        ...
        ctk.CTkLabel(self.frame_oas, text="OAS Folder:").grid(row=0, column=0, sticky="w", padx=(0, 10))
        self.entry_oas = ctk.CTkEntry(self.frame_oas, placeholder_text="Leave empty for default")
        self.entry_oas.grid(row=0, column=1, sticky="ew", padx=(0, 10))
        ctk.CTkButton(self.frame_oas, text="Browse", width=70, command=self._browse_oas).grid(row=0, column=2)
        ...
        self.chk_ignore_br = ctk.CTkCheckBox(self.frame_val, text="Ignore 'Bad Request' Examples by default")
        ...
        ctk.CTkLabel(self.frame_view, text="Theme:").grid(row=0, column=0, sticky="w", padx=(0, 10))
        ...
        ctk.CTkLabel(self.frame_view, text="Font:").grid(row=1, column=0, sticky="w", padx=(0, 10))
        self.cbo_font = ctk.CTkComboBox(self.frame_view, values=["Consolas", "Courier New", "Monaco", "Fira Code", "JetBrains Mono", "Source Code Pro"], width=200)
        self.cbo_font.grid(row=1, column=1, sticky="w", pady=5)
        
        ctk.CTkLabel(self.frame_view, text="Font Size:").grid(row=2, column=0, sticky="w", padx=(0, 10))
        ...
        ctk.CTkLabel(self.frame_logs, text="Generation Logs:").grid(row=0, column=0, sticky="w", padx=(0, 10), pady=2)
        self.cbo_gen_log_theme = ctk.CTkComboBox(self.frame_logs, values=["Light", "Dark"], width=100)
        self.cbo_gen_log_theme.grid(row=0, column=1, sticky="w", pady=2)
        
        ctk.CTkLabel(self.frame_logs, text="Analysis Logs:").grid(row=1, column=0, sticky="w", padx=(0, 10), pady=2)
        self.cbo_analysis_log_theme = ctk.CTkComboBox(self.frame_logs, values=["Light", "Dark"], width=100)
        self.cbo_analysis_log_theme.grid(row=1, column=1, sticky="w", pady=2)
        
        ctk.CTkLabel(self.frame_logs, text="Spectral Output:").grid(row=2, column=0, sticky="w", padx=(0, 10), pady=2)
        self.cbo_spectral_log_theme = ctk.CTkComboBox(self.frame_logs, values=["Light", "Dark"], width=100)
        ...
        ctk.CTkLabel(self.frame_sort, text="Sort Order:").pack(side="left", padx=(0, 10))
        self.cbo_sort = ctk.CTkComboBox(self.frame_sort, values=[opt[0] for opt in self.SORT_OPTIONS], width=150)
        ...
        ctk.CTkLabel(self.frame_iface, text="Default Tab:").grid(row=0, column=0, sticky="w", padx=(0, 10))
        self.cbo_tab = ctk.CTkComboBox(self.frame_iface, values=self.TAB_OPTIONS, width=150)
    ...
    def _create_section_header(self, text: str):
        ...
        ctk.CTkLabel(frame, text=text, font=ctk.CTkFont(weight="bold", size=14)).pack(anchor="w")
