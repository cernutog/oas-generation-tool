...
def _run_webview_docked(html_path, title, x, y, width, height, snap_flag, sync_queue=None, cmd_queue=None):
            ...
            while True:
                try:
                    ...
                    if last_state is None or current != last_state:
                        debug_log(f"Syncing state to UI: {current} (last: {last_state})")
                        ...
                        while retries > 0:
                            try:
                                window.evaluate_js(f'window.setSnapState({str(current).lower()})')
                                last_state = current
                                success = True
                                break
                            except Exception:
...
class DockedDocViewer:
    ...
    def __init__(self, parent, html_path: str, title: str = "API Documentation", 
                 snap_default: bool = True, file_name: str = "", on_snap_callback=None, on_focus_callback=None, on_sync_editor_callback=None):
        self.parent = parent
        ...
        self._on_snap_callback = on_snap_callback  # Called when snap is enabled (for exclusive snap)
        self._on_focus_callback = on_focus_callback # Called when window gains focus while snapped
        self._on_sync_editor_callback = on_sync_editor_callback # Called when viewer requests sync
        self._sync_queue = multiprocessing.Queue() # Queue to receive sync requests from viewer processes
        self._cmd_queue = multiprocessing.Queue()  # Queue to send JS commands to viewer processes
    ...
    def _start_webview(self):
        ...
        self.process = multiprocessing.Process(
            ...
            args=(self.html_path, self._window_title, self.doc_x, self.doc_y, 
                  self.doc_width, self.doc_height, self._snap_flag, self._sync_queue, self._cmd_queue)
        )
    ...
    def _sync_position(self):
        ...
        try:
            ...
            if (current_x == self._last_parent_x and 
                current_y == self._last_parent_y and
                current_width == self._last_parent_width and
                current_height == self._last_parent_height):
            ...
            if self._position_stable_count >= self._STABLE_THRESHOLD:
                ...
                if target_window:
                    ...
                    try:
                        ...
                        is_currently_focused = (active_win and active_win.title == self._window_title)
                        if is_currently_focused and not self._is_focused:
                            # Gained focus!
                            debug_log(f"Viewer (Bound) gained focus: {self._window_title}")
                    ...
                    if abs(doc_x - target_x) > 2 or abs(doc_y - target_y) > 2 or abs(win.height - outer_height) > 2:
